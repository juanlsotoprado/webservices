...
class Servicios extends Conexion{
 ...
 function mensajesMasivosCorreo($parametros) {
	...
	function notificarCorreo($parametros){
		$limite=$parametros->limite;
		$respuesta=array();
	
		if($limite>0){
			try{
				$respuesta=Funciones::enviar_correo_ieu($limite);
	...
	function notificarSMS($parametros){
		$limite=$parametros->limite;
		$respuesta=array();
		
		if($limite>0){
			try{
				$respuesta= Funciones::enviar_sms_ieu($limite);
	...
	function enviarSolicitudConfirmacion($parametros){
		$mensaje= 		$parametros->mensaje;
		$destinatario= 	$parametros->destinatario;
		$cita_id= 	$parametros->id;
		
		if($mensaje!='' && $destinatario!='' && $cita_id!=''){//validación de parametros no vacío
				
			try{
				$respuesta=Funciones::enviarSolicitudConfirmacion2($mensaje, $destinatario, $cita_id);
	...
	function solicitarActualizacion2($parametros){
		...
		if($limite!=''){//validación de parametro no vacío
			if(is_numeric($limite)){//validación de parametro númerico
				try{
					$min_max=Funciones::extraer_minimo_maximo($id);
					$respuesta=Funciones::solicitarActualizacion2($limite, $desde, $min_max[0]['inicio'], $min_max[0]['final']);
					...
					if(count($respuesta)>0){
						...
						for($i=0; $i<count($respuesta); $i++){
							//return Funciones::enviar_correo($respuesta[$i]['id'],$respuesta[$i]['nombre'], $respuesta[$i]['descripcion'], $respuesta[$i]['email'], date('d-m-Y', strtotime($respuesta[$i]['fechacita'])), $respuesta[$i]['horacita'], $respuesta[$i]['correlativo']);
							if(Funciones::enviar_solicitud_actualizacion($respuesta[$i]['usuario'])){
								$enviados++;
							}else{
								$errores++;
							}
							$email_completos.='<br> '.$respuesta[$i]['usuario'].'<br> ';
						}
						Funciones::registrar_actividad($enviados, $errores);
						Funciones::actualizar_minmax($min_max[0]['final']+1, $min_max[0]['final']+$limite, $id);
						$respuesta='[EXITO]:'.($enviados).' <br>'.$email_completos;
					}else{
						return array('error'=>5, 'respuesta'=>array());//no hay notificaciones que enviar
					}
					/*}else{
...
$server = new SoapServer('http://172.17.90.49/webservices/global/global.wsdl');
